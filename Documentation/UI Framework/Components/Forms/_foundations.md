...
All three form types share the same base input styling. The only variation is inline
editing, which uses a plum ring instead of coral for contextual differentiation.
...
- Focus indicators always visible — never suppress the focus ring
...
- **Inline edit exception:** `ring-2 ring-[#403770]` (plum ring, for contextual differentiation inside tables and cards)
- **Modal focus trap:** Tab cycles within the modal; focus returns to the trigger element on close
- **Auto-focus:** first focusable field receives focus on modal or panel open
...
| What | Where |
|------|-------|
| This file | `Documentation/UI Framework/Components/Forms/_foundations.md` |
| Design tokens | `Documentation/UI Framework/tokens.md` |
| Modal shell styling | `Documentation/UI Framework/Components/Containers/modal.md` |
