...
export default function MapV2Container({
  ...
  useEffect(() => {
    if (!map.current || !mapReady) return;
    // Only update if the hover was triggered externally (from search cards)
    // The map's own hover handler sets these directly, so we only need to handle
    // the case where the store is set from outside the map
