            ...
            <div className="flex flex-1 overflow-hidden">
              ...
              <div className="w-[55%] overflow-y-auto p-6 space-y-4">
                ...
                <div>
                  ...
                  />
                  {/* Auto-linked districts & states indicator */}
                  {selectedContacts.length > 0 && (() => {
                    ...
                    // Derive state abbrevs from leaid prefix (first 2 chars = state FIPS)
                    ...
                    )].map((fips) => stateAbbrevMap.get(fips)).filter(Boolean) as string[];
                    if (autoDistricts.length === 0 && autoStates.length === 0) return null;
                    const parts: string[] = [];
                    if (autoDistricts.length > 0) parts.push(autoDistricts.join(", "));
                    if (autoStates.length > 0) parts.push(autoStates.join(", ") + (autoStates.length === 1 ? " (state)" : " (states)"));
                    return (
                      ...
                      {showNewPlanForm ? (
                        <div className="flex items-center gap-1.5 mt-1.5">
                          <input
                            ...
                            className="flex-1 px-2 py-1 text-xs border border-[#C2BBD4] rounded-lg focus:outline-none focus:ring-1 focus:ring-[#F37167] text-[#403770]"
                            autoFocus
                          />
                          <button
                            type="button"
                            ...
                            className="px-2 py-1 text-xs font-medium text-white bg-[#403770] rounded-lg hover:bg-[#322a5a] disabled:opacity-50 transition-colors duration-100"
                          >
                        ...
                        <button
                          ...
                          className="mt-1 text-xs font-medium text-[#403770] hover:text-[#322a5a] transition-colors duration-100"
                        >
                          + New Plan
                        </button>
                      )}
