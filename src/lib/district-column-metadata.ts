...
export const RFP_COLUMNS: ColumnMetadata[] = [
  ...
  { field: "higherGovSourceType", column: "higher_gov_source_type", label: "HigherGov Source Type", description: "HigherGov's source category. Common values: 'sled' (state/local/education solicitations — the bulk of biddable K-12 RFPs), 'federal' (federal agency procurement, occasionally K-12-relevant), 'forecast' (forward-looking announcement of upcoming procurement — NOT yet biddable, use as a 'watch this space' tracking-only signal). DEFAULT FILTER for 'find me RFPs I can bid' / 'open RFPs': higher_gov_source_type IN ('sled','federal'). For 'upcoming RFPs' / 'forecasts to watch' / 'pipeline foreshadowing' filter higher_gov_source_type = 'forecast'. Forecasts often lack due_date and state_abbrev — don't expect those filters to behave like normal RFPs.", domain: "rfp", format: "text", source: "highergov", queryable: true },
...
export const GENERATED_DOCUMENT_COLUMNS: ColumnMetadata[] = [
  ...
  { field: "recipientEmail", column: "recipient_email", label: "Recipient", description: "Signer email for contracts; empty string (NOT NULL) for BOCES quotes — filter with recipient_email = '' not IS NULL.", domain: "crm", format: "text", source: "user", queryable: true },
  { field: "orderTotal", column: "order_total", label: "Order Total", description: "Total dollar value of the quoted/contracted order. Re-renders of a BOCES quote overwrite this value in place — no history is kept; the row always reflects the most recent render.", domain: "crm", format: "currency", source: "user", queryable: true },
  ...
  { field: "schoolYearManual", column: "school_year_manual", label: "SY Manual Entry", description: "True when the rep typed the school year manually instead of picking from the selector — tracks how often the dropdown fails its job (SP6 Addendum 2).", domain: "crm", format: "boolean", source: "user", queryable: true },
  ...
  { field: "opportunityId", column: "opportunity_id", label: "Opportunity ID", description: "FK to opportunities — currently always null (populated once opportunity entry points ship).", domain: "crm", format: "text", source: "user", queryable: true },
  ...
  { field: "sentAt", column: "sent_at", label: "Sent At", description: "When the signature send was initiated (null for BOCES quotes). Full timestamp (timestamptz), not a date. For contracts this equals created_at (row is created at send time).", domain: "crm", format: "date", source: "user", queryable: true },
  { field: "signedAt", column: "signed_at", label: "Signed At", description: "When signing completed. Full timestamp (timestamptz).", domain: "crm", format: "date", source: "user", queryable: true },
...
export const SEMANTIC_CONTEXT: SemanticContext = {
  conceptMappings: {
    bid_worthy_rfps: {
      aggregated:
        "rfps WHERE fullmind_relevance IN ('high','medium') AND due_date >= now() AND set_aside_type IN ('none','small_business') AND higher_gov_source_type IN ('sled','federal') ORDER BY due_date ASC",
      note: "Composite filter for 'what should I bid' / 'actionable RFPs' / 'priority RFPs' / 'open RFPs worth pursuing' rep questions. Excludes forecast rows (those are upcoming-procurement announcements, not yet biddable — see higher_gov_source_type column). Layer rep-scoping with district_pipeline_state for territory ('active' / 'in_plan') or JOIN opportunities for explicit pipeline membership. Add a brief caveat that unclassified rows (classified_at IS NULL) are excluded.",
    },
    bookings: {
      aggregated:
  ...
  warnings: [
    ...
    },
    {
      ...
      severity: "mandatory",
      message:
        "RFP QUERY DEFAULTS: (1) For 'find me RFPs' / 'high-priority RFPs' / 'what should I bid' questions DEFAULT to fullmind_relevance IN ('high','medium'). The 'low' tier is tangential to Fullmind's space (library e-resources, generic edtech, school security) and 'none' is clearly off-topic — both pollute rep-facing results unless the rep explicitly asks for everything. (2) For 'open RFPs' filter due_date >= now() AND higher_gov_source_type IN ('sled','federal'). 'forecast' rows are upcoming-procurement announcements that are NOT yet biddable — exclude them from biddable-RFP questions. Past-due RFPs are retained for ~1 year for historical analysis but reps can no longer bid them. (3) DEFAULT to set_aside_type IN ('none','small_business') unless the rep explicitly asks for set-aside RFPs. Fullmind structurally cannot bid 'minority_owned', 'woman_owned', 'veteran_owned', or 'hub_zone'. (4) Classification columns (fullmind_relevance, keywords, funding_sources, set_aside_type, in_state_only, cooperative_eligible, requires_w9_state) are NULL or default-valued on the unclassified backlog (classified_at IS NULL). Filtering on them silently drops those rows. When a rep DOES filter on relevance, add a brief caveat that unclassified rows are excluded. When the rep does NOT filter on relevance, do NOT add the filter. (5) For 'RFPs at my pipeline districts' filter district_pipeline_state IN ('active','recently_won','recently_lost'). For 'RFPs in my territory plan' filter district_pipeline_state = 'in_plan'. For 'fresh territory' / 'cold districts' filter district_pipeline_state = 'cold'. (6) NEVER expose raw IDs (agency_key, leaid) to reps. JOIN to districts for the district name; use agency_name (not agency_key) when grouping by buyer. Display value_low and value_high as a compact range ('$1.2M – $4.5M'). (7) For ESSER-sunset urgency questions: 'esser' = ANY(funding_sources) AND due_date < '2026-09-30'. Federal ESSER funds expire September 30, 2026. (8) For 'forecasts' / 'upcoming RFPs' / 'pipeline foreshadowing' rep questions: filter higher_gov_source_type = 'forecast'. Forecasts often lack due_date and state_abbrev (early-stage announcements) so urgency/state filters won't behave normally on them — surface a brief caveat when relevant.",
    },
  ],
  excludedTables: [
