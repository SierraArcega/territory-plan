...
export default function ContactsList({ leaid, contacts }: ContactsListProps) {
  ...
  const handleFindContacts = async () => {
    ...
    try {
      await clayLookupMutation.mutateAsync(leaid);
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-3">
        <h3 className="text-sm font-semibold text-[#403770]">Contacts</h3>
        {!isAdding && !editingId && (
                ...
                <div className="flex justify-between items-start">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <span className="font-medium text-[#403770] text-sm">
                        {contact.salutation ? `${contact.salutation} ` : ""}{contact.name}
                      </span>
                      {contact.isPrimary && (
                        <span className="text-[10px] px-1.5 py-0.5 bg-[#F37167] text-white rounded flex-shrink-0">
                          Primary
                        </span>
                      )}
                    </div>
                    ...
                    {contact.email && (
                      <a
                        ...
                        className="text-xs text-[#6EA3BE] hover:underline block mt-0.5"
                      >
                        {contact.email}
                      </a>
                    )}
                    {contact.linkedinUrl && (
                      <a
                        ...
                        className="text-xs text-[#0A66C2] hover:underline flex items-center gap-1 mt-0.5"
                      >
                    ...
                    {(contact.persona || contact.seniorityLevel) && (
                      <div className="flex flex-wrap gap-1 mt-1.5">
                        {contact.persona && (
                          <span className="text-[10px] px-1.5 py-0.5 bg-[#C4E7E6] text-[#403770] rounded">
                            {contact.persona}
                          </span>
                        )}
...
function ContactForm({
  ...
  return (
    <div className="p-2.5 bg-[#C4E7E6]/20 rounded-lg space-y-2">
      <div className="flex gap-2">
        <input
          type="text"
          ...
          className="w-20 px-2 py-1.5 text-xs border border-gray-200 rounded-md focus:outline-none focus:ring-2 focus:ring-[#F37167] focus:border-transparent"
        />
        <input
          type="text"
          value={formData.name}
          ...
          placeholder="Name *"
          className="flex-1 px-2 py-1.5 text-xs border border-gray-200 rounded-md focus:outline-none focus:ring-2 focus:ring-[#F37167] focus:border-transparent"
        />
      </div>
      <input
        type="text"
        ...
        placeholder="Title"
        className="w-full px-2 py-1.5 text-xs border border-gray-200 rounded-md focus:outline-none focus:ring-2 focus:ring-[#F37167] focus:border-transparent"
      />
      <input
        type="email"
      ...
      <div className="flex justify-end gap-2 pt-1">
        <button
          onClick={onCancel}
          className="px-3 py-1 text-xs text-gray-600 hover:text-[#403770]"
        >
          Cancel
        </button>
        <button
          ...
          className="px-3 py-1 text-xs bg-[#F37167] text-white rounded-md hover:bg-[#e05f55] disabled:opacity-50 disabled:cursor-not-allowed"
        >
